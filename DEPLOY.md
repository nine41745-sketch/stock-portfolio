...
3. Push the branch and wait for the Vercel **Preview** deployment.
4. Verify `npm run build` passes. The build runs `scripts/test-critical.mjs` before `next build`.
...
Apply once before the v1.16.0 Preview smoke test:

`supabase/migration_analysis_freshness_v1.16.0.sql`

The migration adds:

- `user_settings.portfolio_updated_at`
- `user_settings.cash_updated_at`
- a holdings trigger that updates the portfolio freshness clock only when symbol, shares, or encrypted cost basis actually changes, plus insert/delete events

It does not alter existing holdings, shares, cost basis, or cash values.
...
If Production smoke fails, stop the release and restore the last known-good Production deployment before making further unrelated changes. Do not create the release tag until Production is confirmed healthy.
