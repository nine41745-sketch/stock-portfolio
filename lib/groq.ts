...
เงินสดที่มี (ข้อมูลอ้างอิงเท่านั้น ไม่ใช่เหตุผลในการแนะนำซื้อ): $${cashBalance.toFixed(2)} (คำนวณได้ ~${canBuyShares} หุ้นถ้าตัดสินใจซื้อ), สัดส่วนเงินสดในพอร์ต: ${cashRatioPct}%
...
=== เกณฑ์การเลือก action (ใช้ตัดสินใจ ห้ามเดาเอง) ===
- SELL_ALL: ขาดทุนหนัก (pnl_pct ต่ำกว่า -15%) ร่วมกับแนวโน้มเทคนิคัลเป็น DOWNTREND หรือมีข่าวลบสำคัญกระทบพื้นฐาน — cut loss
- SELL_PARTIAL: กำไรสูง (pnl_pct เกิน +25%) โดยเฉพาะถ้า RSI เกิน 70 (overbought) หรือราคาชนกรอบบน Bollinger Bands — ควร lock กำไรบางส่วนไว้ก่อน
- BUY: แนวโน้มเป็น UPTREND (ราคา > EMA50 > EMA200), RSI อยู่โซนปกติ (ไม่เกิน 70), ไม่มีข่าวลบสำคัญ และราคายังไม่ไกลจากจุดเข้าที่เหมาะสม — ไม่ใช่แค่เพราะ "มีเงินสดเหลือ" เงินสดเป็นแค่ข้อมูลประกอบว่าซื้อได้กี่หุ้นถ้าตัดสินใจซื้อ ไม่ใช่เหตุผลในการแนะนำซื้อ
- HOLD: ข้อมูลไม่ชัดเจนไปทางใดทางหนึ่ง (เช่น เทรนด์ SIDEWAYS, กำไร/ขาดทุนไม่มาก, ข่าวกลางๆ) หรือ technical indicators คำนวณไม่ได้ (ข้อมูลไม่พอ) ให้ใช้ fundamentals (P/E, 52W High/Low) แทน ถ้ายังไม่มีสัญญาณชัดให้เลือก HOLD ได้ในกรณีนี้เท่านั้น

ห้ามเลือก BUY แค่เพราะมีเงินสดเหลือเยอะ ต้องมีเหตุผลด้านเทคนิคัล/ข่าว/fundamentals รองรับเสมอ
ฟันธงชัดเจนตามเกณฑ์ข้างต้น อย่าเลือก HOLD เพื่อความปลอดภัยถ้าข้อมูลชี้ชัดไปทางอื่น
